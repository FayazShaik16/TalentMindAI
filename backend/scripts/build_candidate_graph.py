...
async def main():
    parser = argparse.ArgumentParser(description="Construct and display Candidate Knowledge Graph relationships.")
    ...
    async with async_session_factory() as session:
        ...
        print(f"Building Knowledge Graph for Candidate ID: {candidate_id}")
        print("=" * 60)
        ...
        graph = final_output["knowledge_graph"]
        nodes = graph["nodes"]
        edges = graph["edges"]
        ...
        for edge in edges[:15]:
            # Try to find target label and source label
